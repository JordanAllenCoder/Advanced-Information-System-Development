...
def delete_airports():
    if 'id' in request.args: 
        id = int(request.args['id'])  # get specific id for desired airport to delete
    else:
        return 'ERROR: No ID provided!'  # error message
    myCreds = creds.Creds()
    conn = create_connection(myCreds.conString, myCreds.userName, myCreds.password, myCreds.dbName)
    cursor = conn.cursor(dictionary=True)
    sqlll = f'DELETE FROM airports WHERE id = {id}'  # sql code to delete a airport using the id
...
def delete_planes():
    if 'id' in request.args: 
        id = int(request.args['id'])  # get specific id for desired plane to delete
    else:
        return 'ERROR: No ID provided!'  # error message
    myCreds = creds.Creds()
    conn = create_connection(myCreds.conString, myCreds.userName, myCreds.password, myCreds.dbName)
    cursor = conn.cursor(dictionary=True)
    planes_sqlll = f'DELETE FROM planes WHERE id = {id}'  # sql code to delete a plane using the id
...
def add_flights():
    ...
    flight_sqll = f"INSERT INTO flights (planeid, airportfromid, airporttoid, date) VALUES ('{planeid}', '{airportfromid}', '{airporttoid}', '{date}')"
    cursor.execute(flight_sqll) # execute the sql code from above and commit the changes using the next line 
    conn.commit()
    return 'Add request was successful'  # reciept
...
def delete_flights():
    if 'id' in request.args: 
        id = int(request.args['id'])  # get specific id for desired flight to delete
    else:
        return 'ERROR: No ID provided!'  # error message
    myCreds = creds.Creds()
    conn = create_connection(myCreds.conString, myCreds.userName, myCreds.password, myCreds.dbName)
    cursor = conn.cursor(dictionary=True)
    flights_sqlll = f'DELETE FROM flights WHERE id = {id}'  # sql code to delete a flight using the id
    cursor.execute(flights_sqlll)
    conn.commit()
    return 'Delete request was successful'  # receipt
